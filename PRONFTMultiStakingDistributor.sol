    ...
    function distribute() external  {
        ...
        for (uint256 i; i < rewardCount; ) {
            ...
            if (currentBalance > minAmount) {
                SafeERC20.safeApprove(IERC20(token), proStaker, currentBalance);  
                IProStaking(proStaker).distributeReward(token, currentBalance);
            }
        ...
        if(currentBalance > minAmount){
            IProStaking(proStaker).distributeReward{value:currentBalance}(address(0), currentBalance);
        }
    }
}
